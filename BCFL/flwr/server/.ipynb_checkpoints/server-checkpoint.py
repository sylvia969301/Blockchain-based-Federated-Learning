...
def ipfsAddFile(fileName):
    ...
    """
    ipfsAdd = subprocess.Popen(args=['ipfs add ' + fileName ], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8')
    print(ipfsAdd)
...
class Server:
    ...
    def fit_round(
        ...
        # hsunchi
        f = open('./aggregated_result', 'w')
        
        # print(jsons.dump(aggregated_result[0]))
        f.write(str(jsons.dump(aggregated_result[0])))
        f.close()
        api = ipfshttpclient.connect('/ip4/127.0.0.1/tcp/5001/http')
        res = api.add('./aggregated_result')
        # print(res)
        ...
        with open(path, 'a+') as f:
            f.write(res['Hash']+'\n')
        # while 1:
        #     localAdd, localAddStt = ipfsAddFile('110_2_BlockChain/Flower/aggregated_result')
        #     if localAddStt == 0:
        #         print('has been added to the IPFS network!')
        #         print('And the hash value of this file is %s'%localAdd)
        #         break
        #     else:
        #         print('Failed to add to the IPFS network!')
